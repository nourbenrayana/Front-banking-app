...
export default function SignUpScreen() {
  ...
  const [withCountryNameButton, setWithCountryNameButton] = React.useState(false);
  ...
  const onSelect = (selectedCountry: Country) => {
    ...
    setSelectedCountryCode(`+${selectedCountry.callingCode[0]}`);
  };
  ...
  const handleSignUp = async () => {
    ...
    try {
     const userDataWithCountryCode = {
      ...formData,
      phone: selectedCountryCode.replace('+', '') + formData.phone, // ✅ correction ici
    };
      ...
      const response = await fetch(`${config.BASE_URL}/api/users/register`, {
        ...
        body: JSON.stringify(userDataWithCountryCode),
      });
